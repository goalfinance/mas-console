<div data-dojo-type="dijit/Dialog" data-dojo-id="authnet.dialogAddForm" id="authnet.dialogAddForm" data-dojo-props="title:'Add new authorized network',href:'dojo/authnet/show_add_form'"></div>
...
<div data-dojo-type="dijit/layout/BorderContainer" id="containerAuthnet"
	...
	<div data-dojo-type="dojo/store/JsonRest"
		...
		data-dojo-props="target:'dojo/authnet/maintains', idProperty:'sid'">
	</div>
	...
	<div data-dojo-type="dijit/layout/ContentPane"
		...
		<table data-dojo-type="dojox.grid.EnhancedGrid" data-dojo-id="authnet.grid"
			...
			plugins:{pagination:true,printer: true}">
			<thead>
				<tr>
					<th field="sid" width="20%">ID</th>
					<th field="authNetId" width="40%">Auth Network ID</th>
	...
	<div data-dojo-type="dijit/layout/ContentPane" id="authnetButtonPane"
			...
			<script type="dojo/on" data-dojo-event="click">
				try{
					...
					dojo.forEach(selectedItems, function(selectedItem){
						var id = authnet.grid.store.getValue(selectedItem, "sid", null);
						if (id == null) return;
						authnet.dialogViewForm.set("href","dojo/authnet/show_view_form?sId=" + id);
		...
		<button data-dojo-type="dijit/form/Button">print preview
			<script type="dojo/on" data-dojo-event="click">
				var cssFiles = [
    				"dojox/grid/tests/enhanced/support/print_style1.css",
    				"dojox/grid/tests/enhanced/support/print_style2.css"
					];
				authnet.grid.exportToHTML({
        title: "Authorized Network List - All",
        cssFiles: cssFiles
    }, function(str){
		var win = window.open();
    win.document.open();
    ...
    authnet.grid.normalizePrintedGrid(win.document);
    win.document.close();
	});
			</script>
		</button>
