 ...
 */
@RestController
@RequestMapping("dojo/authnet/maintains")
public class AuthNetRestController {
	...
	public List<AuthNet> listAuthNetsWithPagination(@RequestHeader(name="Range", required=false) String paginationString,
		...
		Page<AuthorizedNetwork> anPagination = null;
		if (paginationString != null){
			PaginationInfo pInfo = DojoUtils.getPaginationInfo(paginationString);
			anPagination = authorizedNetworkService.findByAuthNetIdAndNameEndsWithAndAuthNetType(
			...
			anPagination = authorizedNetworkService.findByAuthNetIdAndNameEndsWithAndAuthNetType(
					authNetId, name, authNetType, null);
		}
